...
"0,0"; _ref = offset_adjust.split(","), top_adjust = _ref[0], left_adjust = _ref[1]; position = $.validator.get_position(element); if (position === "top") { top = offset.top - 11 - 26 + parseInt(top_adjust); left = offset.left + parseInt(left_adjust) }; if (position === "left") { top = offset.top - 3 + parseInt(top_adjust); left = offset.left - 200 }; if (position === "right") { top = offset.top - 3 + parseInt(top_adjust); left = offset.left + $(element).width() + 20 } return popover.css({ top: top, left: left })
}, get_position: function (element) { return $(element).data("popover-position") || $.validator.validator_settings(element).popoverPosition }, reposition: function (elements) {
    var ele,
